...
class DDC:
    ...
    def __load(self, model_name):
        ...
        with tempfile.TemporaryDirectory() as dirpath:
            ...
            self.__build_sample_model(
                batch_input_length=1024  # could also be self.batch_size
            )  # Multi-output model
    ...
    def predict(self, latent, temp=1, rng_seed=None):
        ...
        for i in range(1000):
            ...
            if temp > 0:
                ...
                nextCharProbs = np.exp(nextCharProbs).astype(np.float64)
                # nextCharProbs = tf.nn.softmax(nextCharProbs, axis=-1).numpy()
                nextCharProbs = (
    ...
    def predict_batch(self, latent, temp=1, rng_seed=None):
        ...
        smiles = np.array([""] * self.batch_input_length, dtype=object).reshape((-1, 1))
        ...
        for i in range(120):
            ...
            if temp > 0:
                ...
                nextCharProbs = np.log(o) / temp
                nextCharProbs = np.exp(nextCharProbs).astype(np.float64)  # .squeeze()

                # Normalize probabilities
                nextCharProbs = (nextCharProbs.T / nextCharProbs.sum(axis=1)).T
            ...
            # update state
            samplevec = tf.reshape(tf.one_hot(sampleidc, depth=self.smilesvec1.dims[-1]), (self.batch_input_length, 1, self.smilesvec1.dims[-1]))
            # append to smile
            # samplechars = np.array([self.smilesvec1._int_to_char[idx] for idx in sampleidc]).reshape((-1, 1))
            samplechars = np.array(list(map(self.smilesvec1._int_to_char.get, sampleidc))).reshape((-1, 1))
            smiles = np.concatenate([smiles, samplechars], axis=1)
